...
from .parallel_utils import ParallelConfig

# --- Helper Functions (No changes needed here) ---

def get_latest_checkpoint(checkpoint_dir: str, logger: Optional[logging.Logger] = None, global_rank: int = 0) -> str:
    """
    ...
    rank_file = os.path.join(latest_dir, f"rank_{global_rank}.pt")
    if not os.path.exists(rank_file):
        if logger:
            logger.info(f"未找到rank {global_rank}的检查点文件: {rank_file}")
        return ""
...
def save_checkpoint(
    ...
    """
    global_rank = parallel_config.global_rank
    dp_rank = parallel_config.dp_rank
    tp_rank = parallel_config.tp_rank
    global_rank_zero = parallel_config.global_rank_zero
    ...
    checkpoint_file = f"rank_{global_rank}.pt"
    ...
    # 4. 通用逻辑：执行保存
    if logger:
        logger.info(f"Rank {global_rank}: 正在为 '{algorithm}' 算法保存检查点到 {checkpoint_path}")
    torch.save(checkpoint, tmp_checkpoint_path)
    os.replace(tmp_checkpoint_path, checkpoint_path)

    # 5. 通用逻辑：处理最佳模型和清理旧检查点
    if is_best and dp_rank == 0:
        ...
        if tp_rank == 0:
            ...
            if logger:
                logger.info(f"Rank {global_rank}: 已保存最佳模型到 {best_path}")
    dist.barrier()
    ...
    if global_rank == global_rank_zero and max_checkpoints > 0:
...
def load_checkpoint(
    ...
    if global_rank == global_rank_zero:
