...
from utils.load_data_model_tp import *  # 从 utils/load_data_model.py 中导入函数
...
from utils.shard_utils_tp import *
from crosstrain.algorithms.algorithms_tp import *
...
def train(model, dataloader, optimizer, scheduler,
          ...
          dp_group=None, tp_group=None, dp_rank=0, tp_rank=0):
    """
    训练过程：
    - 模型以 train 模式运行
    - 实现传、算分离的分块同步策略
    ...
    if args.algorithm == 'diloco':
        ...
        # 为每个分片创建跟踪信息
        shard_tracker = {}
        global_param_counts, global_byte_counts = print_shard_param_counts_from_shards(shard_params, logger, tp_group, tp_rank, dp_rank, device)
    ...
    for epoch_idx in range(epoch, args.epochs):
        ...
        for batch in train_dataloader:
            ...
            if micro_step % args.gradient_accumulation_steps == 0:
                ...
                _ = tp_aware_clip_grad_norm_(model.parameters(), max_norm=1.0, tp_group=tp_group, norm_type=2.0)
                # model.clip_grad_norm_(1.0)
                scaler.step(optimizer)
                scaler.update()
                scheduler.step()
                optimizer.zero_grad(set_to_none=True)
                # scaler._grad_scaler = None  # 重置GradScaler缓存（适用于PyTorch 1.10+）
                    ...
                    for shard_idx in range(num_shards):
                        ...
                        if shard_tracker[shard_idx]["next_receive_step"] == global_step:
                            comm_time = sync_streaming_diloco(
                                model, shard_tracker, shard_idx, dp_group, logger,
...
def main():
    ...
    # --- NEW: Build TP/DP groups (non-intrusive) ---
    ...
    # create TP groups: same dp_rank, different tp_rank
    tp_groups = [dist.new_group(ranks=[dp * tp_size + i for i in range(tp_size)]) for dp in range(dp_size)]
    tp_group = tp_groups[dp_rank]

    # create DP groups: same tp_rank, different dp_rank
    dp_groups = [dist.new_group(ranks=[tp + i * tp_size for i in range(dp_size)]) for tp in range(tp_size)]
    ...
    model.to(device)
    from utils.load_data_model_tp import apply_tp_sharding  # NEW helper
    model = apply_tp_sharding(model, tp_group, args.tp_size, logger)
    adjust_attention_heads_for_tp(model, tp_size, logger)
    ...
    if args.weight_decay > 0:
        optimizer = AdamW(model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay, foreach=False)
    else:
        optimizer = AdamW(model.parameters(), lr=args.learning_rate, foreach=False)
    ...
    # 计算通信延迟
    comm_delay = calc_comm_delay(model, dp_world_size, logger, simulated_bandwidth_mbps=args.bandwidth)
    ...
    try:
        train(model, (train_dataloader, eval_dataloader), optimizer,
              ...
              task_type, args,
              dp_group=dp_group, tp_group=tp_group, dp_rank=dp_rank, tp_rank=tp_rank)
    except Exception as e:
